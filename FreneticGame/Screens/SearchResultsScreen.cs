    ...
    public class SearchResultsScreen : MenuScreen
    {
        ...
        public override void OnSelectEntry(int entryIndex)
        {
                ...
                (entryIndex < availableSessions.Count))
            {
                ...
                try
                {
                    IAsyncResult asyncResult = Microsoft.Xna.Framework.Net.NetworkSession.BeginJoin(
                    ...
                    busyScreen.OperationCompleted += LoadGameplayScreen;
        ...
        internal void SessionsFound(object sender, OperationCompletedEventArgs e)
        {
            ...
            if (availableSessions != null)
            {
                foreach (AvailableNetworkSession availableSession in
                    availableSessions)
                {
                    if (availableSession.CurrentGamerCount < WorldManager.MaximumPlayers)
                    {
                        MenuEntries.Add(availableSession.HostGamertag + " (" +
                            availableSession.CurrentGamerCount.ToString() + "/" +
                            WorldManager.MaximumPlayers.ToString() + ")");
                    }
        ...
        private void LoadGameplayScreen(object sender, OperationCompletedEventArgs e)
        {
            ...
            {
                GameplayScreen gameplayScreen = new GameplayScreen(networkSession);
